...
class TestLabelGeneration:
    ...
    def test_labels_are_binary(self, trending_up_df, labels_module):
        labels = labels_module(trending_up_df, asset='gold')
        unique_vals = set(labels.unique())
        assert unique_vals <= {0.0, 1.0}, f"Labels must be 0 or 1, got {unique_vals}"

    def test_labels_match_dataframe_index(self, small_ohlcv_df, labels_module):
        labels = labels_module(small_ohlcv_df, asset='gold')
        assert len(labels) == len(small_ohlcv_df)
        assert labels.index.equals(small_ohlcv_df.index)

    def test_uptrend_has_positive_labels(self, labels_module):
        n = 200
        ...
        labels = labels_module(df, asset='gold')
        assert labels.sum() > 0, "Steep uptrend should generate positive labels"

    def test_downtrend_has_fewer_positive_labels(self, labels_module):
        n = 200
        dates = pd.date_range('2024-01-01', periods=n, freq='15min')
        ...
        labels = labels_module(df_down, asset='gold')
        ...
        up_labels = labels_module(df_up, asset='gold')
        assert labels.sum() <= up_labels.sum(), "Downtrend should have fewer positive labels"

    def test_labels_with_custom_params(self, small_ohlcv_df, labels_module):
        labels_default = labels_module(small_ohlcv_df, asset='gold')
        labels_tight = labels_module(
            small_ohlcv_df,
    ...
    def test_labels_gold_vs_silver_params(self, small_ohlcv_df, labels_module):
        gold_labels = labels_module(small_ohlcv_df, asset='gold')
        silver_labels = labels_module(small_ohlcv_df, asset='silver')
        assert len(gold_labels) == len(silver_labels)

    def test_labels_last_bars_are_zero(self, small_ohlcv_df, labels_module):
        labels = labels_module(small_ohlcv_df, asset='gold')
        max_bars = 6
        assert labels.iloc[-max_bars:].sum() == 0, "Last max_bars labels should be 0"

    def test_custom_tp_higher_creates_more_labels(self, small_ohlcv_df, labels_module):
        labels_low = labels_module(
            small_ohlcv_df, take_profit_pct=0.05, stop_loss_pct=0.01, asset='gold'
        )
        labels_high = labels_module(
    ...
    def test_labels_series_name(self, small_ohlcv_df, labels_module):
        labels = labels_module(small_ohlcv_df, asset='gold')
        assert labels.name == 'target'

    def test_labels_with_empty_bars(self, labels_module):
        n = 20
        dates = pd.date_range('2024-01-01', periods=n, freq='15min')
        df = pd.DataFrame({
