...
class TestHigherTimeframeFeatures:
    ...
    def test_htf_columns_added(self, multi_timeframe):
        df = self._make_mtf_df()
        result = multi_timeframe["add_higher_timeframe_features"](df, ['1h_close', '1h_high', '1h_low'], '1h')
    ...
    def test_htf_trend_is_binary(self, multi_timeframe):
        df = self._make_mtf_df()
        result = multi_timeframe["add_higher_timeframe_features"](df, ['1h_close', '1h_high', '1h_low'], '1h')
        valid = result['htf_1h_trend'].dropna()
        assert set(valid.unique()) <= {0, 1}

    def test_htf_atr_non_negative(self, multi_timeframe):
        df = self._make_mtf_df()
        result = multi_timeframe["add_higher_timeframe_features"](df, ['1h_close', '1h_high', '1h_low'], '1h')
        valid = result['htf_1h_atr'].dropna()
        assert (valid >= 0).all()

    def test_htf_rsi_in_range(self, multi_timeframe):
        df = self._make_mtf_df()
        result = multi_timeframe["add_higher_timeframe_features"](df, ['1h_close', '1h_high', '1h_low'], '1h')
        valid = result['htf_1h_rsi'].dropna()
        assert (valid >= 0).all() and (valid <= 100).all()

    def test_htf_no_close_col_returns_unchanged(self, multi_timeframe):
        n = 100
        dates = pd.date_range('2024-01-01', periods=n, freq='15min')
        df = pd.DataFrame({
            'open': np.ones(n) * 2000,
        ...
        result = multi_timeframe["add_higher_timeframe_features"](df, ['1h_high'], '1h')
        assert result.shape == df.shape

    def test_does_not_modify_original(self, multi_timeframe):
        df = self._make_mtf_df()
        original_cols = list(df.columns)
        multi_timeframe["add_higher_timeframe_features"](df, ['1h_close', '1h_high', '1h_low'], '1h')
...
class TestAllMultiTimeframeFeatures:
    ...
    def test_adds_htf_features(self, multi_timeframe):
        ...
        result = multi_timeframe["add_all_multi_timeframe_features"](df)
        assert any(c.startswith('htf_') for c in result.columns)

    def test_no_htf_data_returns_original_columns(self, multi_timeframe):
    ...
    def test_multiple_timeframes(self, multi_timeframe):
        dates = pd.date_range('2024-01-01', periods=200, freq='15min')
        close = 2000.0 + np.random.randn(200).cumsum() * 2
        df = pd.DataFrame({
            'open': close - 1,
