...
class TestFairValueGaps:
    ...
    def test_fvg_columns_added(self, small_ohlcv_df, smc_features):
        result = smc_features["detect_fair_value_gaps"](small_ohlcv_df)
        assert 'fvg_bullish' in result.columns
        assert 'fvg_bearish' in result.columns
        assert 'fvg_size' in result.columns

    def test_fvg_bullish_is_binary(self, small_ohlcv_df, smc_features):
        result = smc_features["detect_fair_value_gaps"](small_ohlcv_df)
        unique_bull = set(result['fvg_bullish'].dropna().unique())
        assert unique_bull <= {0, 1}

    def test_fvg_bearish_is_binary(self, small_ohlcv_df, smc_features):
        result = smc_features["detect_fair_value_gaps"](small_ohlcv_df)
        unique_bear = set(result['fvg_bearish'].dropna().unique())
        assert unique_bear <= {0, 1}

    def test_fvg_does_not_modify_original(self, small_ohlcv_df, smc_features):
        original_cols = list(small_ohlcv_df.columns)
        smc_features["detect_fair_value_gaps"](small_ohlcv_df)
        assert list(small_ohlcv_df.columns) == original_cols

    def test_fvg_rolling_count_columns(self, small_ohlcv_df, smc_features):
        result = smc_features["detect_fair_value_gaps"](small_ohlcv_df)
        assert 'fvg_bullish_count' in result.columns
        assert 'fvg_bearish_count' in result.columns

    def test_fvg_size_non_negative(self, small_ohlcv_df, smc_features):
        result = smc_features["detect_fair_value_gaps"](small_ohlcv_df)
        assert (result['fvg_size'] >= 0).all(), "FVG size should be non-negative"

    def test_fvg_custom_threshold(self, small_ohlcv_df, smc_features):
        detect = smc_features["detect_fair_value_gaps"]
        result_tight = detect(small_ohlcv_df, threshold=0.01)
        result_loose = detect(small_ohlcv_df, threshold=0.0001)
...
class TestBreakOfStructure:
    ...
    def test_bos_columns_added(self, small_ohlcv_df, smc_features):
        result = smc_features["detect_break_of_structure"](small_ohlcv_df)
        assert 'bos_bullish' in result.columns
        assert 'bos_bearish' in result.columns

    def test_bos_distance_columns(self, small_ohlcv_df, smc_features):
        result = smc_features["detect_break_of_structure"](small_ohlcv_df)
        assert 'distance_from_swing_high' in result.columns
        assert 'distance_from_swing_low' in result.columns

    def test_bos_is_binary(self, small_ohlcv_df, smc_features):
        result = smc_features["detect_break_of_structure"](small_ohlcv_df)
        assert set(result['bos_bullish'].dropna().unique()) <= {0, 1}
        assert set(result['bos_bearish'].dropna().unique()) <= {0, 1}

    def test_bos_rolling_count(self, small_ohlcv_df, smc_features):
        result = smc_features["detect_break_of_structure"](small_ohlcv_df)
        assert 'bos_bullish_count' in result.columns
        assert 'bos_bearish_count' in result.columns

    def test_bos_does_not_modify_original(self, small_ohlcv_df, smc_features):
        original_cols = list(small_ohlcv_df.columns)
        smc_features["detect_break_of_structure"](small_ohlcv_df)
        assert list(small_ohlcv_df.columns) == original_cols
...
class TestLiquiditySweeps:
    ...
    def test_sweep_columns_added(self, small_ohlcv_df, smc_features):
        result = smc_features["detect_liquidity_sweeps"](small_ohlcv_df)
    ...
    def test_sweep_is_binary(self, small_ohlcv_df, smc_features):
        result = smc_features["detect_liquidity_sweeps"](small_ohlcv_df)
        assert set(result['liquidity_sweep_high'].dropna().unique()) <= {0, 1}
        assert set(result['liquidity_sweep_low'].dropna().unique()) <= {0, 1}

    def test_sweep_count_column(self, small_ohlcv_df, smc_features):
        result = smc_features["detect_liquidity_sweeps"](small_ohlcv_df)
        assert 'liquidity_sweep_count' in result.columns

    def test_sweep_strength_non_negative(self, small_ohlcv_df, smc_features):
        result = smc_features["detect_liquidity_sweeps"](small_ohlcv_df)
...
class TestOrderBlocks:
    ...
    def test_order_block_columns_added(self, small_ohlcv_df, smc_features):
        result = smc_features["detect_order_blocks"](small_ohlcv_df)
    ...
    def test_order_block_is_binary(self, small_ohlcv_df, smc_features):
        result = smc_features["detect_order_blocks"](small_ohlcv_df)
        assert set(result['order_block_bullish'].dropna().unique()) <= {0, 1}
        assert set(result['order_block_bearish'].dropna().unique()) <= {0, 1}

    def test_order_block_strength_non_negative(self, small_ohlcv_df, smc_features):
        result = smc_features["detect_order_blocks"](small_ohlcv_df)
...
class TestPremiumDiscount:
    ...
    def test_premium_discount_columns(self, small_ohlcv_df, smc_features):
        result = smc_features["add_premium_discount_zones"](small_ohlcv_df)
    ...
    def test_position_is_between_0_and_1(self, small_ohlcv_df, smc_features):
        result = smc_features["add_premium_discount_zones"](small_ohlcv_df)
        valid = result['premium_discount_position'].dropna()
        assert (valid >= 0).all() or (valid <= 1).all(), "Position should be between 0 and 1"

    def test_premium_and_discount_are_binary(self, small_ohlcv_df, smc_features):
        result = smc_features["add_premium_discount_zones"](small_ohlcv_df)
...
class TestMarketStructure:
    ...
    def test_market_structure_columns(self, small_ohlcv_df, smc_features):
        result = smc_features["add_market_structure"](small_ohlcv_df)
    ...
    def test_bullish_bearish_structure(self, small_ohlcv_df, smc_features):
        result = smc_features["add_market_structure"](small_ohlcv_df)
        assert 'bullish_structure' in result.columns
        assert 'bearish_structure' in result.columns

    def test_structure_is_binary(self, small_ohlcv_df, smc_features):
        result = smc_features["add_market_structure"](small_ohlcv_df)
...
class TestAllSMCFeatures:
    ...
    def test_adds_many_columns(self, small_ohlcv_df, smc_features):
        result = smc_features["add_all_smc_features"](small_ohlcv_df)
        assert len(result.columns) > len(small_ohlcv_df.columns)

    def test_original_columns_preserved(self, small_ohlcv_df, smc_features):
        result = smc_features["add_all_smc_features"](small_ohlcv_df)
        for col in ['open', 'high', 'low', 'close', 'volume']:
            assert col in result.columns

    def test_does_not_modify_original(self, small_ohlcv_df, smc_features):
        original_len = len(small_ohlcv_df.columns)
        smc_features["add_all_smc_features"](small_ohlcv_df)
        assert len(small_ohlcv_df.columns) == original_len

    def test_row_count_preserved(self, small_ohlcv_df, smc_features):
        result = smc_features["add_all_smc_features"](small_ohlcv_df)
        assert len(result) == len(small_ohlcv_df)

    def test_no_new_nans_beyond_rolling_windows(self, small_ohlcv_df, smc_features):
        result = smc_features["add_all_smc_features"](small_ohlcv_df)
